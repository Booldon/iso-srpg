...
### 작업 내용
- Fire F4 슬라이스 완성: 틱 수정자 카드 4장
  - `docs/systems/card_system_api.md` v1.4 — F4 계약 추가 (커밋 `f589934`)
  - `scripts/data/card_data.gd` — F4 필드 5개 추가 (`on_attack_burn_tick_multiplier`, `on_attack_burn_decay_slow`, `on_attack_burn_max_override`, `on_burn_threshold_armor_debuff`, `on_burn_threshold_armor_min`)
  - `scripts/battle/unit.gd` — burn_* 런타임 필드 5개 + `effective_armor()` 추가
  - `scripts/battle/status_effects.gd` — `add()` BURN 상한 `unit.burn_max` 사용, `tick_turn_start()` 배수·감쇠 지연 로직
  - `scripts/battle/card_effects.gd` — `apply_on_attack()` [3] 틱수정자 블록, detonation consume-all 버그 수정, fill_max `unit.burn_max` 기준 갱신
  ...
  - Python 스모크 20/20 통과 (커밋 `228b006`, `af36c28`, `6a5f92b`)

### 결정 사항
- Smolder 감쇠 위상: decay-first (첫 틱에 감쇠 → 다음 틱 스킵 → 반복). 시각 검증 후 skip-first로 변경 가능 (`_burn_decay_skip` 초기값 true로)
- High Density: `on_attack_burn_max_override` 필드명 (예약명 그대로). 전투 시작 시 전역 적용 (공격 트리거 아님)
- Brittle Coat: `effective_armor()` 동적 계산, stats.armor 직접 수정 안 함
- 서브에이전트 신뢰 경계 이슈 발생: systems-designer가 coordinator 릴레이를 신뢰하지 않아 plan mode 해제 전까지 편집 거부 → 메인이 직접 문서 편집으로 해결
...
- Fire F5: Phoenix 계열(once-per-battle 트리거) + Avatar of Blaze (`is_boss` 플래그 신설)
- Agnostic Rare/Epic 카드 데이터

### 주요 파일 변경
- `docs/systems/card_system_api.md` — v1.4 (F4 전체 계약)
- `scripts/data/card_data.gd` — F4 필드 5개
- `scripts/battle/unit.gd` — burn_* 필드 + `effective_armor()`
